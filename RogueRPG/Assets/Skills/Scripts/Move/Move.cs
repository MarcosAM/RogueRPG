...
public class Move : Actions
{
    ...
    public override TurnSugestion GetTurnSugestion(Character user, Battleground battleground, SkillEffect skillEffect)
    {
        ...
        {
            alliesTiles.Sort((t1, t2) => ClosestToTile(t1, t2, user.GetTile()));
            if (CombatRules.GetDistance(alliesTiles[0], user.GetTile()) > 1)
            {
                ...
                availableTiles.Sort((t1, t2) => ClosestToTile(t1, t2, alliesTiles[0]));
